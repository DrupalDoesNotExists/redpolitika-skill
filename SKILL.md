...
Навык для ИИ-агентов: как писать YAML-правила для [Редполитики](https://redpolitika.ru).
...
**Server-side** — содержат `ref` или позиционные/контекстные методы, или LLM/NER/POS/плагин.

Сервер всегда выполняет ВСЕ правила (и клиентские, и серверные).
...
2. **severity**: 1–3 cosmetic, 4–6 significant, 7–10 critical. Для helper-правил ставь 0 или опусти (0 по умолчанию)
3. **category**: `cleanliness` — грамматика, стиль, канцелярит; `readability` — читаемость, типографика
4. **suggestion**: пиши конкретные инструкции, что делать — не "плохо", а "замените X на Y"
5. **regex**: всегда проверяй на RE2-совместимость. Сложные паттерны — разбивай на композицию
6. **exclude**: используй для исключения ложных срабатываний вместо сложного regex
7. **ref**: выноси общие подпаттерны в helper-правила (severity=0) для переиспользования
8. **fiх**: если правило только находит, но не исправляет — не пиши fix (будет suggestion, но без автоисправления)
9. **examples**: всегда давай bad/good примеры — они видны пользователю
...
9. examples имеют bad/good где нужно
10. Если есть fix — он совместим с detect

---

*Этот навык покрывает CE-версию Редполитики. EE и плагины расширяют методы детекции, но схема правил остаётся той же.*
